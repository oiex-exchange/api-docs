...
Due to the reasons of high latency and poor stability, it is not recommend to access the Oiex API through a proxy.
...
Way to request an address: Accessing the server address `api.oiex.com` such as `api.oiex.com/trade/api/v1/order`.

API accesskey: There is the Access Key in the API Key you applied for.

Timestamp (nonce): The timestamp of your application's request, 13-bit milliseconds. Oiex will verify the validity of your API request based on this timestamp.

Signature: A value calculated by the signature to ensure the signature is valid and has not been tampered. Oiex uses HmacSHA256.
...
`https://api.oiex.com/trade/api/v1/getOrder?accesskey={AccessKey}&market={Market}&nonce={Timestamp}&id={OrderId}&signature={Signature}`
...
nonce|integer|true|N/A|13-bit milliseconds|
coin|string|true|N/A|Coin name|btc,eth,ltc...
address|string|true|N/A|Withdrawal address|Only your certified addresses at Oiex are supported
